...
CString CUtils::SaltedMD5Hash(const CString& sPass, const CString& sSalt) { return CString(sPass + sSalt).MD5(); }

CString CUtils::SaltedSHA256Hash(const CString& sPass, const CString& sSalt) { return CString(sPass + sSalt).SHA256(); }
...
bool CUtils::GetBoolInput(const CString& sPrompt, bool bDefault) { return CUtils::GetBoolInput(sPrompt, &bDefault); }
...
void CUtils::SetMessageTags(CString& sLine, const MCString& mssTags)
{
    ...
    if (!mssTags.empty()) {
        ...
        for (const auto& it : mssTags) {
            ...
            sTags += it.first;
            if (!it.second.empty()) sTags += "=" + it.second.Escape_n(CString::EMSGTAG);
        }
        sLine = "@" + sTags + " " + sLine;
    }
}
