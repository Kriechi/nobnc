...
{
    putClient(":irc.bnc.no 464 " + nick() + " :Password required");
    putClient(":irc.bnc.no NOTICE AUTH :*** "
...
void NoClient::readLine(const NoString& data)
{
    ...
    if (!isAttached()) { // The following commands happen before authentication with NoBNC
        if (command.equals("PASS")) {
            ...
            authUser();
            return; // Don't forward this msg. NoBNC has already registered us.
            ...
            authUser();
            return; // Don't forward this msg. NoBNC will handle nick changes until auth is complete
    ...
    if (command.equals("CAP")) {
        ...
        // we don't want anything enabled that NoBNC does not support.
        return;
    }
    ...
    if (command.equals("NO")) {
        ...
        // All PONGs are generated by NoBNC. We will still forward this to
        // the ircd, but all PONGs from irc will be blocked.
        if (line.length() >= 5)
            putClient(":irc.bnc.no PONG irc.bnc.no " + line.substr(5));
        else
            putClient(":irc.bnc.no PONG irc.bnc.no");
...
void NoClient::refuseLogin(const NoString& reason)
{
    ...
    putClient(":irc.bnc.no 464 " + nick() + " :" + reason);
...
void NoClient::acceptLogin(NoUser* user)
{
    ...
    if (!d->sNetwork.empty()) {
        ...
        // If no "default" network, try "user" network. It's for compatibility with early network stuff in NoBNC, which
        ...
        if (d->network && d->user->networks().size() > 1) {
            ...
            putStatusNotice("Selecting network [" + d->network->name() +
                            "]. To see list of all configured networks, use /no ListNetworks");
            putStatusNotice(
            "If you want to choose another network, use /no JumpNetwork <network>, or connect to NoBNC with username " +
            d->user->userName() + "/<network> (instead of just " + d->user->userName() + ")");
        }
    } else {
        putStatusNotice("You have no networks configured. Use /no AddNetwork <network> to add one.");
    }
...
void NoClient::putModuleNotice(const NoString& module, const NoString& line)
{
    ...
    NO_DEBUG("(" << fullName() << ") BNC -> CLI [:" + d->user->statusPrefix() + ((module.empty()) ? "status" : module) + "!no@bnc.no NOTICE "
                 << nick() << " :" << line << "]");
    d->socket->write(":" + d->user->statusPrefix() + ((module.empty()) ? "status" : module) + "!no@bnc.no NOTICE " +
                     nick() + " :" + line + "\r\n");
}
...
{
    putClient(":irc.bnc.no CAP " + nick() + " " + response);
}
...
void NoClient::handleCap(const NoString& line)
{
    ...
    if (sSubCmd.equals("LS")) {
        ...
        ssOfferCaps.insert("bnc.no/server-time-iso");
        ssOfferCaps.insert("bnc.no/batch");
        ssOfferCaps.insert("bnc.no/self-message");
        ...
        for (const NoString& sToken : vsTokens) {
            ...
            bool bAccepted = ("multi-prefix" == cap) || ("userhost-in-names" == cap) || ("bnc.no/server-time-iso" == cap) ||
                             ("bnc.no/batch" == cap) || ("bnc.no/self-message" == cap);
        ...
        for (const NoString& sToken : vsTokens) {
            ...
            if ("multi-prefix" == cap) {
                ...
                d->hasUhNames = bVal;
            } else if ("bnc.no/server-time-iso" == cap) {
                d->hasServerTime = bVal;
            } else if ("bnc.no/batch" == cap) {
                d->hasBatch = bVal;
            } else if ("bnc.no/self-message" == cap) {
        ...
        if (d->hasServerTime) {
            ...
            ssRemoved.insert("bnc.no/server-time-iso");
        }
        if (d->hasBatch) {
            d->hasBatch = false;
            ssRemoved.insert("bnc.no/batch");
        }
        if (d->hasSelfMessage) {
            d->hasSelfMessage = false;
            ssRemoved.insert("bnc.no/self-message");
        }
        NoString sList = "";
        ...
        putClient(":irc.bnc.no 410 " + nick() + " " + sSubCmd + " :Invalid CAP subcommand");
    }
}
