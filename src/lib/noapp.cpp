...
NoString NoApp::tag(bool includeVersion, bool bHTML)
{
    ...
    NoString sAddress = bHTML ? "<a href=\"https://nobnc.github.io\">https://nobnc.github.io</a>" : "https://nobnc.github.io";

    if (!includeVersion) {
        return "NoBNC - " + sAddress;
    }

    NoString sVersion = version();

    return "NoBNC - " + sVersion + " - " + sAddress;
}
    ...
    while (true) {
        ...
        case ConfigNeedRehash:
            ...
            if (rehashConfig(error)) {
                ...
                broadcast("NoBNC is in some possibly inconsistent state!", true);
            }
            break;
...
{
#ifndef HAVE_LIBSSL
    No::PrintError("NoBNC was not compiled with ssl support.");
    return false;
#else
...
NoString NoAppPrivate::makeConfigHeader()
{
    return "// WARNING\n"
           "//\n"
           "// Do NOT edit this file while NoBNC is running!\n"
...
// Listen
    ...
    do {
        while (true) {
            ...
            if (listenPort == 6667) {
                ...
                No::printStatus(false, "use NoBNC's web interface, you might want to use another port.");
    ...
    No::getInput("Real name", answer, "https://nobnc.github.io");
    ...
    do {
        ...
        if (NoFile::Exists(d->configFile)) {
            if (!file.TryExLock(d->configFile)) {
                No::printStatus(false, "NoBNC is currently running on this config.");
                fileOk = false;
            } else {
                file.Close();
    ...
    No::printMessage("");
    No::printMessage("To connect to this NoBNC you need to connect to it as your IRC server", true);
    ...
    No::printMessage("/server <nobnc_server_ip> " + port + " " + user + ":<pass>", true);
    No::printMessage("");
    ...
    file.UnLock();
    return fileOpen && No::getBoolInput("Launch NoBNC now?", true);
}
...
bool NoAppPrivate::doRehash(NoString& error)
{
    ...
    if (!NoFile::Exists(configFile)) {
        ...
        No::printMessage("Restart NoBNC with the --makeconf option if you wish to create this config.");
        return false;
    }
    ...
    if (!pFile->TryExLock()) {
        error = "NoBNC is already running on this config.";
        No::printStatus(false, error);
        delete pFile;
        return false;
    }
    ...
    if (tSavedVersion < tCurrentVersion) {
        No::printMessage("Found old config from NoBNC " + sSavedVersion + ". Saving a backup of it.");
        backupConfigOnce("pre-" + NoString(NO_VERSION_STR));
    } else if (tSavedVersion > tCurrentVersion) {
        No::printError("Config was saved from NoBNC " + sSavedVersion + ". It may or may not work with current NoBNC " + noApp->version());
    }
...
NoApp::TrafficStatsMap NoApp::trafficStats(TrafficStatsPair& Users, TrafficStatsPair& App, TrafficStatsPair& Total)
{
    TrafficStatsMap ret;
    ulonglong uiUsers_in, uiUsers_out, uiApp_in, uiApp_out;
    const std::map<NoString, NoUser*>& users = noApp->userMap();

    uiUsers_in = uiUsers_out = 0;
    uiApp_in = bytesRead();
    uiApp_out = bytesWritten();
    ...
    for (NoSocket* socket : d->manager.sockets()) {
        ...
        if (user) {
            ...
            uiApp_in += info.bytesRead();
            uiApp_out += info.bytesWritten();
        }
    }

    Users = TrafficStatsPair(uiUsers_in, uiUsers_out);
    App = TrafficStatsPair(uiApp_in, uiApp_out);
    Total = TrafficStatsPair(uiUsers_in + uiApp_in, uiUsers_out + uiApp_out);
...
public:
    ...
    {
        // This is only needed when NoBNC shuts down:
